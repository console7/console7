...
- **Tests:** (a) cloud-gcp — assert the readiness gate. The branch gates at **both** points: `kubeEngineRunner.Run` waits pod `condition=Ready` + per-session proxy `condition=Available` before the first exec, and `Provision` waits pod `condition=Ready` before returning. (Both are intentional defence-in-depth — `Provision`'s gate makes the sandbox injectable; `Run` re-checks cheaply and adds the proxy.) Test the `Run` gate at minimum; a `Provision`-gate assertion is belt-and-suspenders. **No mutex is held across either wait** (an earlier "Provision holds `p.mu`" concern was unfounded — there is no such lock in the provider or orchestrator). (b) scm-github — `pullRequestPermissions` includes `contents:read` and intersects correctly; (c) scm-github — `CreatePullRequest` retries `isTransientRefRace` and gives up after the bound (the real opener is the `ghApp` adapter, so test the extracted retry helper / refactor the loop to be unit-testable).
...
- **Design (as built):** a small **auth-proxy runs as its own (per-session) Deployment** — alongside the
  ...
  `ANTHROPIC_VERTEX_PROJECT_ID`, `CLOUD_ML_REGION`, plus the engine's `HTTP_PROXY`/`HTTPS_PROXY` **emptied**
  so the in-cluster engine→auth-proxy hop is direct (verified F2c-2d: claude-code 1.0.44 IGNORES `NO_PROXY`
  and CONNECT-tunnels every request through `HTTP_PROXY`, which can't reach a bare ClusterIP; clearing the
  proxy env makes it dial direct, and the default-deny NetworkPolicy stays the authoritative confinement).
  **No bearer is ever delivered into the sandbox pod; the sandbox holds no GCP credential and stays metadata-free.**
...
- **cloud-gcp changes (done):** `engineRunScript` (Vertex lane) — **DROPPED `CLOUDSDK_AUTH_ACCESS_TOKEN`**
  ...
  `authProxyEndpoint`, and the engine's proxy env cleared — F2c-2d, since the engine ignores `NO_PROXY`);
  render the per-session auth-proxy Deployment + Service + NetworkPolicy (modelled on the Squid renderer).
...
- **Tests:** render test (engine env points at the auth-proxy + skip-auth + the proxy env **cleared**, **no**
