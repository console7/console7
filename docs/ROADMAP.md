...
| ~~#16~~ ✅ | ~~`chainHash` has no secret, so a workload-SA holder who reads the tail can craft a valid forward record / fork the chain~~ | **CLOSED — Phase 2 (E2 + E1b).** Per-record lineage verification (`Sink.VerifyLineage` → `VerifyRecordPayload`) now runs in the production verify path: a forged/forked record's chain hash is recomputable, but its position-bound NHI signature (E1) cannot be re-minted, so it fails verification. Plus a fail-fast tail-corruption recompute in `evidence-gcs` preflight, and `Count()` filtering stray objects; production retention-LOCK is the boundary control. |
