...
| **CO-17** | Code quality, maintainability, tech debt | **Adopt** | Lint (`gofmt`, `go vet`, `golangci-lint`) wired as a blocking gate ✓ — `golangci-lint run ./...` (whole-tree) gates every PR + push (`.github/workflows/go.yml`, config `.golangci.yml`: complexity/duplication/length/nesting/doc/security axes); per-package test-coverage floors ratchet via `scripts/coverage-gate.sh` (CO-15). Tech debt + calibrated lint exceptions tracked in **[`docs/RISKS.md`](../RISKS.md)**. Local defence-in-depth: the `pre-pr-review` skill runs the same config before push. (Delivered by the CO-17 enforcement plan, `docs/dev/code-quality-enforcement-plan.md`.) |
